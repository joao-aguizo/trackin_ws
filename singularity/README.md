...
```bash
[BarnBot]$ cd ~/user_ros_workspace/
[BarnBot]$ catkin init
[BarnBot]$ catkin build
```
...
```bash
./wrapper.sh
[BarnBot] user@hostname:~$ source ~/user_ros_workspace/devel/setup.bash
[BarnBot] user@hostname:~$ roslaunch cpr_agriculture_gazebo agriculture_world.launch platform:=husky
```
